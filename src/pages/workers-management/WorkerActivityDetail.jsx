...
const WorkerActivityDetail = () => {
    ...
    async (silent = false) => {
      ...
      try {
        ...
        const [mainRes, allRes] = await Promise.all([
          api.get(`/staff-activities/worker/${workerId}?${qs}`),
          api.get(`/staff-activities/worker/${workerId}?${qsAll}`),
        ]);
        ...
        toast.error(
          err.response?.data?.message || "Failed to load worker activity",
        );
      } finally {
        setLoading(false);
      }
  ...
  const jobActivities = useMemo(
    () =>
      allActivities.filter((a) =>
        ["job_view", "job_action"].includes(a.activityType),
      ),
    [allActivities],
  );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-purple-50/30 to-blue-50/20">
        <div className="text-center">
          ...
          <p className="text-slate-500">Loading worker analytics…</p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Worker data not found.</p>
          <button
            onClick={() => navigate(-1)}
  ...
  const serviceTypeLabel = worker?.service_type
    ? worker.service_type.charAt(0).toUpperCase() + worker.service_type.slice(1)
    : "";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/20 to-blue-50/10 p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {serviceTypeLabel && (
                    <span className="flex items-center gap-1 text-xs text-blue-700 bg-blue-100 border border-blue-200 px-2.5 py-0.5 rounded-full font-medium">
                      <Briefcase className="w-3 h-3" /> {serviceTypeLabel}
                    </span>
                  )}
                  {worker?.role && (
                    <span className="flex items-center gap-1 text-xs text-purple-700 bg-purple-100 border border-purple-200 px-2.5 py-0.5 rounded-full font-medium">
                      <Wrench className="w-3 h-3" /> {worker.role}
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 mt-1.5 flex-wrap text-xs text-slate-400">
                  ...
                  {worker?.createdAt && (
                    <span className="flex items-center gap-1">
                      ...
                      Joined: {fmtDate(worker.createdAt)}
                    </span>
                  )}
                </div>
              </div>
      ...
      <Card delay={0.24}>
        <div className="bg-gradient-to-r from-blue-500/10 via-cyan-500/10 to-teal-500/10 -m-5 p-5 rounded-t-2xl border-b-2 border-blue-200 mb-4">
          ...
          <p className="text-xs text-slate-600 mt-1 font-medium">
            In-depth analysis of staff app usage per screen
          </p>
        </div>
      ...
      <Card delay={0.25}>
        ...
        {formSubmissions.length === 0 ? (
          <EmptyState text="No enquiry/form submissions recorded for this worker" />
        ) : (
      ...
      <Card delay={0.27}>
        ...
        {jobActivities.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-slate-400">
              Job views and actions will appear here
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
      ...
      <Card delay={0.29}>
        <div className="bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 -m-5 p-5 rounded-t-2xl border-b-2 border-indigo-200 mb-4">
          ...
          />
          <p className="text-xs text-slate-600 mt-1 font-medium">
            Complete staff app journey from login to logout — with detailed
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3 mb-3 pb-3 border-b border-indigo-100">
                      ...
                      <button
                        onClick={() => {
                          const box = document.getElementById(`wsession-${i}`);
                    ...
                    <div id={`wsession-${i}`} className="space-y-2">
...
export default WorkerActivityDetail;
